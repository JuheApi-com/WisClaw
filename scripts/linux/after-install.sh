...
if [ -x /opt/WisClaw/wisclaw ]; then
    ln -sf /opt/WisClaw/wisclaw /usr/local/bin/wisclaw 2>/dev/null || true
fi

# Create symbolic link for openclaw CLI
OPENCLAW_WRAPPER="/opt/WisClaw/resources/cli/openclaw"
...
if ! { [[ -L /proc/self/ns/user ]] && unshare --user true; }; then
    ...
    chmod 4755 '/opt/WisClaw/chrome-sandbox' || true
else
    chmod 0755 '/opt/WisClaw/chrome-sandbox' || true
fi
...
if apparmor_status --enabled > /dev/null 2>&1; then
    APPARMOR_PROFILE_SOURCE='/opt/WisClaw/resources/apparmor-profile'
    APPARMOR_PROFILE_TARGET='/etc/apparmor.d/wisclaw'
...
echo "WisClaw has been installed successfully."
